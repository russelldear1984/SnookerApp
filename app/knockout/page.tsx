          ...
          <div key={match.id} className="card p-5">
            ...
            {match.side1Id && match.side2Id && (
              <div className="mt-3 grid grid-cols-2 gap-2">
                <button className="rounded-lg border border-emerald-200 bg-emerald-50 px-2 py-1 text-sm text-emerald-800" onClick={() => setKnockoutWinner(match.id, match.side1Id!)}>
                  {getEntryName(match.side1Id)} won
                </button>
                <button className="rounded-lg border border-emerald-200 bg-emerald-50 px-2 py-1 text-sm text-emerald-800" onClick={() => setKnockoutWinner(match.id, match.side2Id!)}>

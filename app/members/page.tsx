...
export default function MembersPage() {
  ...
  return (
    <AppShell title="Club Manager">
      ...
      {state.players.map((p)=> <div key={p.id} className="card flex items-center justify-between p-4"><div><p className="text-2xl font-semibold text-slate-900">{p.name}</p><p className="text-slate-500">Wins: {p.wins} | Losses: {p.losses}</p></div><button onClick={()=>deletePlayer(p.id)} className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-red-700">Delete</button></div>)}

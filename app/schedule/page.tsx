...
export default function SchedulePage() {
  ...
  return (
    <AppShell title="Schedule & Bookings">
      <div className="card p-5">
        ...
        <form className="mt-3 space-y-3" onSubmit={onSubmit}>
          ...
          <label className="text-sm font-semibold text-slate-600">Player / Team 1</label>
          <select className="input" name="side1" required>
            <option value="">Select Player / Team 1</option>
            {entryOptions.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}
          </select>

          <label className="text-sm font-semibold text-slate-600">Player / Team 2</label>
          <select className="input" name="side2" required>
            <option value="">Select Player / Team 2</option>
            {entryOptions.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}
          </select>
